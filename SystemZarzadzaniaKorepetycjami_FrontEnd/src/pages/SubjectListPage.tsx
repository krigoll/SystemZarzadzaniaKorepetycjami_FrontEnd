...
const SubjectListPage: React.FC = () => {
  ...
  const handleAddNewSubject = async () => {
    try {
      ...
      console.log(`Dodano nowy przedmiot: ${subjectName}`);
      setAddSubject(false);
      refresh();
    } catch (err) {
  ...
  const handleAddNewCategory = async () => {
    ...
    try {
      ...
      console.log(
        `Dodano nową kategorię "${subjectCategoryName}" do przedmiotu: ${selectedSubject}!`
      );
      setAddCategory(false);
      refresh();
    } catch (err) {
  ...
  const handleAddNewLevel = async () => {
    ...
    try {
      ...
      console.log(
        `Dodano nowy poziom "${subjectLevelName}" do kategorii: ${selectedCategory}!`
      );
      setAddLevel(false);
      refresh();
    } catch (err) {
      console.error('Błąd przy dodawaniu poziomu:', err);
    }
  };
  ...
  const handleDeletingCategory = async () => {
    ...
    try {
      ...
      console.log(`Usunięto "${selectedCategory}" z ${selectedSubject}`);
      setDeleteCategory(false);
      refresh();
    } catch (err) {
  ...
  const handleDeletingLevel = async () => {
    ...
    try {
      ...
      console.log(`Usunięto "${selectedLevel}" z ${selectedCategory}`);
      setDeleteLevel(false);
      refresh();
    } catch (err) {
      console.error('Błąd przy usuwaniu poziomu', err);
    }
  };
          ...
          <div className="subject-management-container">
            <div className="subject-management-column">
              ...
              {Object.keys(subjectsData).map((subject) => (
                <button
                  key={subject}
                  className={`subject-management-button ${
                ...
                {getLevels()
                  ...
                  .map((levelData) => (
                    <button
                      key={levelData.id}
                      className={`subject-management-button ${
                        selectedLevel === levelData.level ? 'selected' : ''
                      }`}
              ...
              <div className="subject-management-column">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (value.length > 50) {
                      alert('Maksymalna długość nazwy to 50 znaków.');
                    } else {
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (value.length > 50) {
                      ...
                      setSubjectCategoryName(e.target.value);
                    }
                  }}
            ...
            {deleteCategory && (
              <div className="subject-management-column">
                <p>
                  Usuń kategorię {selectedCategory} z przedmiotu{' '}
                  {selectedSubject}.
                </p>
                <button
                  className="subject-management-add-button"
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (value.length > 50) {
                      ...
                      setSubjectLevelName(e.target.value);
                    }
                  }}
